...
// ✨ FIX: Initial project settings configured for correct blank/default behavior.
const initialProjectSettings = JSON.parse(JSON.stringify({
    projectName: "", // Remains blank for user entry
    clientName: "", // Remains blank for user entry
    projectAddress: "",
    projectCity: "",
    ownerName: "",
    ownerAddress: "",
    ...
    activeTrades: ["General"], // Revert to General auto-selected as requested
}));

// ✨ FIX: Initial estimate items cleared for a fresh start, as they will be generated by the wizard.
...
function renderItems() {
    ...
    estimateItems.forEach(item => {
        ...
        if (!item.laborEntries || !Array.isArray(item.laborEntries)) {
            // Default to the first selected trade, or 'General' if none selected yet
            const defaultTrade = activeTrades.length > 0 ? activeTrades[0] : 'General';
            item.laborEntries = [{ id: `lab_${Date.now()}`, trade: defaultTrade, rateRole: 'Journeyman', hours: 0, otDtMultiplier: 1.0 }];
        }
...
function startDetailedEstimate() {
    ...
    // activeTrades is already set to ["General"] in initialProjectSettings, so no need to explicitly set it here.
